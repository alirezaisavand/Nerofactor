from pathlib import Path
import os
import argparse

import torch
import numpy as np
import trimesh
from tqdm import tqdm
import open3d as o3d

# ---- your existing utils (we rely on their behavior) ----
# mask_depth_to_pts returns camera-frame points from (mask, depth, K)
# project_points expects OpenCV-style W2C extrinsics and K
from dataset.database import (
    parse_database_name, get_database_split,
    GlossySyntheticDatabase, NeRFSyntheticDatabase, get_database_eval_points
)
from utils.base_utils import (
    mask_depth_to_pts, project_points, pose_apply, pose_inverse # pose_apply -> transform_points_pose under the hood
)
# NOTE: We do NOT use your old pose_inverse here for NeRF; we compute w2c from c2w via inv+S.

# -------------------------
# Convention bridge helpers
# -------------------------

def to_4x4(M):
    """Accept (3,4) or (4,4); return (4,4) float32 homogeneous."""
    M = np.asarray(M, dtype=np.float32)
    if M.shape == (4, 4):
        return M
    if M.shape == (3, 4):
        return np.vstack([M, np.array([[0, 0, 0, 1]], dtype=np.float32)])
    raise ValueError(f"Pose must be (3,4) or (4,4), got {M.shape}")

# OpenGL->OpenCV camera-frame flip (x right, y up, -z fwd) -> (x right, y down, +z fwd)
S4 = np.diag([1.0, -1.0, -1.0, 1.0]).astype(np.float32)
S3 = np.diag([1.0, -1.0, -1.0]).astype(np.float32)

def gl_c2w_to_cv_w2c(c2w_gl_4x4):
    """
    Convert OpenGL c2w to OpenCV w2c for use with OpenCV-style projection.
    Formula: w2c_cv = S * inv(c2w_gl)
    """
    return S4 @ np.linalg.inv(c2w_gl_4x4)

# -------------------------
# Your existing rasterizer (OpenCV-style)
# -------------------------

def rasterize_depth_map(mesh, pose_w2c_cv, K, shape, near=5e-1, far=1e2):
    """
    Keep your original OpenCV-style path:
      - 'pose_w2c_cv' is OpenCV world->camera (3x4 or 4x4)
      - 'project_points' expects OpenCV conventions
    """
    import nvdiffrast.torch as dr

    vertices = np.asarray(mesh.vertices, dtype=np.float32)
    faces = np.asarray(mesh.faces, dtype=np.int32)

    # Ensure (3,4)
    M = np.asarray(pose_w2c_cv, dtype=np.float32)
    if M.shape == (4, 4):
        M = M[:3, :]
    elif M.shape != (3, 4):
        raise ValueError(f"pose_w2c_cv must be (3,4) or (4,4), got {M.shape}")

    pts, depth = project_points(vertices, M, K)  # OpenCV projection (from your utils)
    h, w = shape
    # to NDC (OpenCV raster path you had)
    pts[:, 0] = (pts[:, 0] * 2 - w) / w
    pts[:, 1] = (pts[:, 1] * 2 - h) / h

    z = (depth - near) / (far - near)
    z = z * 2 - 1
    pts_clip = np.concatenate([pts, z[:, None]], 1)

    pts_clip = torch.from_numpy(pts_clip.astype(np.float32)).cuda()
    indices = torch.from_numpy(faces.astype(np.int32)).cuda()
    pts_clip = torch.cat([pts_clip, torch.ones_like(pts_clip[..., 0:1])], 1).unsqueeze(0)

    ctx = dr.RasterizeCudaContext()
    rast, _ = dr.rasterize(ctx, pts_clip, indices, (h, w))  # [1,h,w,4]
    depth_img = (rast[0, :, :, 2] + 1) / 2 * (far - near) + near
    mask = rast[0, :, :, -1] != 0
    return depth_img.detach().cpu().numpy(), mask.detach().cpu().numpy().astype(bool)

# -------------------------
# Chamfer utility (unchanged)
# -------------------------

def nearest_dist(pts0, pts1, batch_size=512):
    pts0 = torch.from_numpy(pts0.astype(np.float32)).cuda()
    pts1 = torch.from_numpy(pts1.astype(np.float32)).cuda()
    pn0, pn1 = pts0.shape[0], pts1.shape[0]
    dists = []
    for i in tqdm(range(0, pn0, batch_size), desc='evaluating...'):
        dist = torch.norm(pts0[i:i + batch_size, None, :] - pts1[None, :, :], dim=-1)
        dists.append(torch.min(dist, 1)[0])
    dists = torch.cat(dists, 0)
    return dists.cpu().numpy()

# -------------------------
# Point set generators (NeRF Synthetic)
# -------------------------

def get_mesh_eval_points_nerf(database, mesh_path):
    """
    Rasterize predicted mesh from NeRF camera views, then back-project to world.
    Steps per view:
      A) For rasterization (OpenCV pipeline kept): w2c_cv = S * inv(c2w_gl)
      B) depth_pr, mask_pr = rasterize_depth_map(mesh, w2c_cv, K, (H,W))
      C) pts_cam_cv = mask_depth_to_pts(mask_pr, depth_pr, K)
      D) pts_cam_gl = (S3 @ pts_cam_cv^T)^T
      E) pts_world  = pose_apply(c2w_gl, pts_cam_gl)
    """
    assert isinstance(database, NeRFSyntheticDatabase)
    _, _, test_ids = get_database_split(database, 'test')
    mesh = trimesh.load_mesh(mesh_path)
    pbar = tqdm(total=len(test_ids), desc='mesh->pts (NeRF)')

    pts_chunks = []
    for test_id in test_ids:
        K = database.get_K(test_id)
        c2w_gl = to_4x4(database.get_pose(test_id))
        H, W, _ = database.get_image(test_id).shape

        # A) OpenCV W2C for rasterization
        w2c_cv_4 = gl_c2w_to_cv_w2c(c2w_gl)
        # B) depth from mesh
        depth_pr, mask_pr = rasterize_depth_map(mesh, w2c_cv_4, K, (H, W))
        # C) back-project in OpenCV camera model
        pts_cam_cv = mask_depth_to_pts(mask_pr, depth_pr, K)        # (N,3) CV cam
        # D) to GL camera frame
        pts_cam_gl = (S3 @ pts_cam_cv.T).T
        # E) to world
        pts_world = pose_apply(c2w_gl, pts_cam_gl)
        if pts_world.shape[1] == 4:
            pts_world = pts_world[:, :3] / np.maximum(1e-8, pts_world[:, 3:4])
        pts_chunks.append(pts_world.astype(np.float32))
        pbar.update(1)

    pts_pr = np.concatenate(pts_chunks, 0).astype(np.float32)
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(pts_pr.astype(np.float64))
    downpcd = pcd.voxel_down_sample(voxel_size=0.01)
    return np.asarray(downpcd.points, np.float32)

# -------------------------
# Main script (NeRF Synthetic)
# -------------------------

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--mesh', type=str, required=True)
    parser.add_argument('--object', type=str, required=True)
    parser.add_argument('--batch_size', type=int, default=1024)
    args = parser.parse_args()

    # Build NeRF Synthetic DB
    database = parse_database_name(f'nerf/{args.object}', 'data/nerf_synthetic')

    # GT and predicted point clouds in WORLD coordinates
    pts_gt = get_database_eval_points(database)
    pts_pr = get_mesh_eval_points_nerf(database, args.mesh)

    print(f"pts_gt min: {pts_gt.min():.5f}, max: {pts_gt.max():.5f}, shape: {pts_gt.shape}")
    print(f"pts_pr min: {pts_pr.min():.5f}, max: {pts_pr.max():.5f}, shape: {pts_pr.shape}")

    # Chamfer
    dist_gt = nearest_dist(pts_gt, pts_pr, args.batch_size)
    dist_pr = nearest_dist(pts_pr, pts_gt, args.batch_size)
    chamfer = 0.5 * (np.mean(dist_gt) + np.mean(dist_pr))

    stem = Path(args.mesh).stem
    results = f'{stem} {chamfer:.5f}'
    print(results)
    os.makedirs('data', exist_ok=True)
    with open('data/geometry_nerf.log', 'a') as f:
        f.write(results + '\n')

if __name__ == "__main__":
    main()
