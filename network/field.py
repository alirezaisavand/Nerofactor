...
class MCShadingNetwork(nn.Module):
    ...
    def compute_tangent_bitangent_flat(self, normals, sources):
        ...
        if sources is None:
            ...
            normals_normalized = F.normalize(normals, p=2, dim=-1, eps=1e-6)
            dot = torch.abs(torch.sum(normals_normalized * ref_dir.unsqueeze(0).expand(normals.size(0), -1), dim=-1))
            zero_tangent_mask = dot > 0.99
            if zero_tangent_mask.any():
                ...
                tangent[zero_tangent_mask] = torch.cross(normals[zero_tangent_mask], ref_dir.unsqueeze(0).expand(normals.size(0), -1), dim=-1)
