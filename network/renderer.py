...
class NeROShapeRenderer(nn.Module):
    ...
    def nvs(self, pose, K, h, w):
        ...
        for ri in range(0, rn, trn):
            ...
            with torch.no_grad():
                rays_o, rays_d, near, far, human_poses = self._process_nerf_ray_batch(cur_ray_batch, imgs_info[
                    'poses']) if is_nerf else self._process_ray_batch(cur_ray_batch, imgs_info['poses'])
    ...
    def train_step(self, step):
        ...
        outputs['loss_fg'] = self.compute_rgb_loss(train_ray_batch['rgbs'] * masks_reshaped, masks_reshaped * outputs['fg_rgb'])
    ...
    def render_core(self, rays_o, rays_d, z_vals, human_poses, cos_anneal_ratio=0.0, step=None, is_train=True,
        ...
        outputs['loss_curv'] = curvature.mean()
        if use_refscores:
...
class NeROMaterialRenderer(nn.Module):
    ...
    def train_step(self, step):
        ...
        if self.cfg['reg_mat']:
            ...
            shade_outputs['loss_mat_reg'] = self.shader_network.anisotropic_regularization(
                pts, normals, shade_outputs['source'], shade_outputs['mx'], shade_outputs['my'], shade_outputs['alpha'], shade_outputs['F0'],
                shade_outputs['kd'], shade_outputs['ks'], shade_outputs['f_d_sum'], shade_outputs['f_s_sum'],
                shade_outputs['L_spec'], shade_outputs['rotation'], step
            )
            # shade_outputs['loss_mat_reg'] = self.shader_network.material_regularization(
    ...
    def test_step(self, index):
        ...
        output_keys = {'rgb_gt': 3, 'rgb_pr': 3, 'specular_light': 3, 'diffuse_light': 3,
                       ...
                       'f_d_sum': 3, 'f_s_sum': 3, 'tangents': 3, 'bitangents': 3, 'normals': 3, 'source': 3,'L_spec': 3, }
        ...
        for ri in range(0, rn, trn):
            ...
            if torch.sum(hit_mask) > 0:
                ...
                outputs_cur['source'][hit_mask] = shade_outputs['source'].float()
